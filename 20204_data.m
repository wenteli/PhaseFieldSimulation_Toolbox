...
data = readtable('sol_20204.csv', 'HeaderLines',1);
...
for i=1:nx
    ...
    elseif i==nx
          ...
          if abs(ezz(i-1)-ezz(i))>tol && abs(ezz(i+1)-ezz(i))<tol 
             left=i;
             ezz_left_bound=[ezz_left_bound left];
             fprintf('left=%d;   ', left);
          elseif abs(ezz(i+1)-ezz(i))>tol && abs(ezz(i-1)-ezz(i))<tol 
             right=i;
             ezz_right_bound=[ezz_right_bound right];
             fprintf('right=%d;\r\n', right);
          end
